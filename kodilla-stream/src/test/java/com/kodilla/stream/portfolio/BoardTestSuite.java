...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        double average =   project.getTaskLists().stream()
                ...
                .map(np -> np.getCreated().until(LocalDate.now()).getDays())
...
//        int taskQuantity = 0;
//        List<Task> inProgress = project.getTaskLists().get(1).getTasks();
//        for (Task task : inProgress) {
//            expectedAverage =+ task.getCreated().until(LocalDate.now()).getDays();
//            taskQuantity++;
//        }
